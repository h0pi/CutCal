...
public class CountriesController : BaseCRUDController<CountryResponse, CountrySearchObject, CountryInsertRequest, CountryUpdateRequest, ICountryService>
{
    ...
    [HttpPost]
    [Authorize(Roles = RoleNames.Admin)]
    public override Task<ActionResult<CountryResponse>> Insert([FromBody] CountryInsertRequest request) => base.Insert(request);

    [HttpPut("{id:int}")]
    [Authorize(Roles = RoleNames.Admin)]
    public override Task<ActionResult<CountryResponse>> Update(int id, [FromBody] CountryUpdateRequest request) => base.Update(id, request);
